...
// Proxy genérico para todas las rutas API restantes
app.use('/api', async (req, res) => {
  try {
    ...
    // Configurar opciones de la solicitud con valores predeterminados
    const options = {
      method,
      url,
      ...
      validateStatus: function(status) {
        return true; // Permitir cualquier código de estado para manejar errores manualmente
      }
    };
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`⚡️ Servidor proxy mejorado ejecutándose en http://localhost:${PORT}`);
  console.log(`⚡️ Reenviando solicitudes a ${BFF_SERVER}`);
  console.log(`⚡️ Usuario de prueba disponible: test@example.com / password`);
  console.log(`⚡️ Funciones implementadas:`);
  console.log(`   - Verificación de email (pre-login)`);
  console.log(`   - Login con usuario y contraseña`);
  console.log(`   - Refresh token`);
  console.log(`   - Proxy genérico para todas las demás rutas API`);
});
